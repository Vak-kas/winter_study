...
%% bit_data 불러오기
original_bit_data= readFile("./plain.txt", keytext, ivtext);
...
% ASCII 코드를 문자로 변환
textString = char(decimalValues, 8)';


% 암호화된 문자 파일로 저장
encryptedFilename = 'encrypted_data.enc';
fileID = fopen(encryptedFilename, 'wb');
fwrite(fileID, textString);
fclose(fileID);

% 복호화
result = readFile("./encrypted_data.enc", "dec", keytext, ivtext);
...
%%
function binaryNumArray = readFile(filename, keytext, ivtext)

    command = sprintf('python DES3.py "%s" "%s" "%s"', filename, keytext, ivtext);
%     disp(command)
    [status, textString] = system(command);

    % 결과 확인 및 오류 처리
    if status ~= 0
        error('Error occurred during the encryption/decryption process.');
    end
    % 문자열의 각 문자를 ASCII 코드 값으로 변환
    asciiValues = double(textString);
    
    % ASCII 코드 값을 2진수로 변환
    binaryStrings = dec2bin(asciiValues);
