...
%% Prepare
% n = rng(10);
nSymbol = 1000000; % 비트 수
SNR_dB = 20; %SNR값
SNR_linear = 10^(SNR_dB/10); %Signal Power
count = 0; %전체 전송 bit 기입 
...
count = sum(bit_data ==recovered_bit_data);
...
disp(["SNR_dB = ", num2str(SNR_dB), "BPSK = ", num2str(BPSK_err_count/BPSK_count), ...
    ...
    "AVG = " , num2str((BPSK_err_count + QPSK_err_count + QAM_err_count) / (BPSK_count + QPSK_count + QAM_count)), "count= ", count] );
